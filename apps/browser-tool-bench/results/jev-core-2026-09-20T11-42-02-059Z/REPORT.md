...
**批次状态：故障已复现并修复，桌面实例尚未加载修改。已证实控制条会误用并断开 Agent 的 CDP 会话。局部修复通过 78 项测试和 12 次真实 Chrome 对照验证，无需重启 Gateway 或关闭 Chrome。速度评测仍为 8/36 次；诊断运行不计入模型评测。详见 GATEWAY-DIAGNOSIS.md。**
