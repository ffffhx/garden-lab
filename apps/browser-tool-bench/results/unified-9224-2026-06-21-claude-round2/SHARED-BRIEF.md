...
- **agent-browser / bb-browser / devtools-mcp**：连用户的 **CDP 9224** 测试 Chrome（已登录 GitHub `ffffhx`、已装 content-script-only 扩展 Bench Badge `jkmndkochpgaleoechlemhdhbikdecnf`，无 SW 不在 /json）。靶场把它导航到 localhost:4399；外场打真实网站。不要自起新浏览器。
- **playwright-cli**：用它**自管的浏览器**（不 attach 9224，attach 装扩展的 9224 必崩）。靶场连 localhost:4399 没问题；外场 R01-R09 里需要真实登录态/扩展的几题（R02/R06 等）自管浏览器没有 → 如实记 N-R。

## 特殊任务

- T09：改 `extension-sample/manifest.json` 版本到 1.0.1 → reload → 验证徽标 v1.0.1 → **改回 1.0.0 并 reload 恢复**。企业 9224 可能拦 reload，遇到如实记。
- T10a：读 9224 的 GitHub 通知数（只读，禁写）。playwright-cli 自管→N-R。
...
- 证据存到 `results/unified-9224-2026-06-21-claude-round2/<你的工具名>/`。
