...
## 附录：Bifrost 和 Clash 在功能上到底差在哪

正文里它俩都以“应用层代理入口”的身份出现，端口一个 8899 一个 7897，看起来像同类工具。但从功能上看，它们解决的是两个不相干的问题：

```text
Clash   ：换路径，不换终点，不看内容 —— 管“流量走哪条路”
Bifrost ：换终点，拆开内容改写       —— 管“内容怎么改”
```

### 各自回答的问题

Clash Verge（核心是 Mihomo）回答的是：**这个请求该走哪条路？**

- 按域名 / IP / 地区分流：Claude 走英国节点、国内网站直连、内网走别的线。
- 节点测速切换、故障转移、TUN 全局接管。
- 全程只看 TLS 握手里明文的 SNI 域名和 IP 端口，不拆流量——像快递分拣中心，只看地址，不拆包裹。

Bifrost 回答的是：**这个请求 / 响应的内容能不能被我拦下来改掉？**

- 把 `https://code.coze.cn` 的页面指到本机 `localhost:8080`——前端不用部署，就能在真实域名、真实登录态下调试本地代码。
- mock 接口返回、改请求头、注入脚本、抓包看明文、模拟弱网。
- 这一切都要求拆开 TLS 看到明文——所以装它必须信任它的 CA 证书。

### 为什么 Clash 替代不了“域名代理到本地”

拿一条真实的 Bifrost 规则拆解：

```text
https://code.coze.cn/  http://localhost:8080
includeFilter://m:get
excludeFilter://code.coze.cn/api/
```

意思是：code.coze.cn 的页面请求走本地 dev server，`/api/` 继续走线上，且只拦 GET。这需要三个 Clash 架构上没有的能力：

1. **按路径和方法分流**。URL 路径和请求方法在 TLS 加密层里面。Clash 不解密，对一条 `code.coze.cn` 的连接只能整域名一刀切，劈不开“页面走本地、接口走线上”——而这恰恰是本地调试玩法的核心：本地前端代码要配线上真实后端。
2. **协议改写**。浏览器发起的是 https，本地 dev server 只会明文 http。Bifrost 在中间对浏览器扮演 coze.cn 完成 TLS 握手，拆出明文再转给本地。Clash 只搬运字节：假设强行把 `code.coze.cn:443` 的连接引到 `127.0.0.1:8080`，浏览器会隔着管道向 dev server 要 TLS 握手，后者拿不出 coze.cn 的证书，握手当场失败。
3. **域名语义保持**。浏览器的 Cookie、登录态、同源策略全部跟着地址栏域名走。要让浏览器“真心相信”自己在访问 code.coze.cn，必须有人向它出示能通过验证的 coze.cn 证书——全世界只有两种角色做得到：持有真证书的真服务器，或一个你手动信任过其 CA 的中间人。Clash 两者都不是：它是管道工，铺好管道就退到一边，让两端自己握手；它没有 CA、不签证书，架构上根本不参与这场验证。

### CA 证书是分水岭

判断一个代理工具需不需要你信任 CA，看一条就够：

```text
换路径不换终点（Clash 走任意节点）：
  TLS 端到端完好，代理只见密文 -> 不需要 CA
换终点（Bifrost 把 coze.cn 指到 localhost）：
  新终点证明不了自己是原域名，必须中间人拆开重新包装 -> 必须信任 CA
```
...
| | Clash Verge / Mihomo | Bifrost |
|---|---|---|
| 核心能力 | 分流路由、节点管理 | 请求 / 响应拦截与改写 |
| 看得见 HTTPS 内容吗 | 看不见（无需 CA） | 看得见（必须信任 CA） |
| 改变的是什么 | 流量的**路径** | 流量的**内容** |
| 没有它会怎样 | 有些网站访问不了或很慢 | 网站照常访问，但线上页面调不到本地代码 |
| 典型用户 | 所有需要分流 / 出海的人 | 开发者 |

最后回到正文的主题：两者都会去抢“系统代理”这个入口，后开的覆盖先开的。要同时用——一边翻墙一边调试——就是第 3 节的做法：Bifrost 在前面拆内容改写，改完的流量用 `proxy://` 显式串到 Clash 的 7897 再出去，各管各的一层。
