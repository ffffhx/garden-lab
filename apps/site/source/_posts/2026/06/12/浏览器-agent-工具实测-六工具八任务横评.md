...
- **playwright-cli**：纯自动化最稳（综合通过率最高、几乎零 eval 自救），T10c 也能 attach 9223 读到登录态，说明 attach 能力本身是有的；但 attach 装了多个扩展的真实 profile 会确定性触发 service_worker 断言崩（外场两轮均复现，非一次性 flake），所以它是"自启浏览器回归测试"的首选，不是"真实 profile 主力"的首选。
