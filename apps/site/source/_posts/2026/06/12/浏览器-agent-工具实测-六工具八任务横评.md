...
**② Codex 轮（gpt-5.5）**：同样"顺序 + 独立 subagent"，多测了 Codex 专属的 `@chrome` / `@browser`。这轮还抓到了 **output token**（总 token 九成多是缓存输入、很便宜，真实生成量看 output）。耗时是各 subagent 自报、**不可跨宿主与 Claude 轮直接比**。
...
第 1 节那两张结果总表（T01-T20 与 R01-R09）都只跑了一轮（agent-browser 同日两轮）。为收方差，我又补跑了一轮：2026-06-20 我用 **Claude Code 主控、每工具一个干净 Subagent、顺序复用同一台 9223 测试 Chrome**（playwright-cli 在这轮因 attach 装了扩展的 9223 必崩、改用自管浏览器），把外场 R01-R09 和靶场 T01-T20 **各独立跑了两轮**专门收方差。这轮只比四个真实 CLI/MCP 工具（Codex 专属 `@chrome`/`@browser` 无等价物，未纳入），与同日 Codex 轮互不参考。
