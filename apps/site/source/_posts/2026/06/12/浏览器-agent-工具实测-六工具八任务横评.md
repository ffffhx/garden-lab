...
| 标签页 / 窗口 / target | 多 tab、新窗口、popup 弹出窗口（`window.open` 出来的独立网页窗口，如 OAuth 登录窗；**不含 alert/confirm 这类原生对话框**——那是页面触发、却在 DOM 之外、只能由 CDP 的 Page/Browser/Fetch 等 domain 单独处理的模态框，另算一种薄控制面），以及后台 target（service worker / background page） | **前台多 tab / 新窗口 / popup**：四个 CDP 工具（agent-browser、bb-browser、DevTools MCP、playwright-cli）都能枚举并切换，@chrome 走 `chrome.tabs` 也管得了；**后台 target**（service worker / background page）要 CDP 的 Target 域才看得到，只有自管浏览器的 agent-browser、DevTools MCP、playwright-cli（自管 context）拿得到，@chrome / @browser 的扩展 runtime 只读、够不着 | playwright-cli 一旦改成 attach 企业 Chrome，枚举到扩展的 service_worker target 反而触发内部断言、直接崩（自管时没问题） |
