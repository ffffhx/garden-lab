...
- **playwright-cli**：纯自动化最稳（综合通过率最高、几乎零 eval 自救），attach 9223 本身能成（T10c 已证）；但它接管真实 9223 的稳定性**随版本/环境漂移**——评测当轮外场两轮都崩、当时记为 service_worker 断言，而 2026-06-22 复测（playwright-cli 0.1.14）已复现不出：9223 有打开的页面时 attach 稳定成功（并存 6 个扩展 service_worker 也不崩），只在无任何页面时崩于 `Browser.setDownloadBehavior`（详见 4.7）。可预测性不足，所以仍定位为"自启浏览器回归测试"的首选、而非"真实 profile 主力"。
...
第 2 节那两张结果总表（T01-T20 与 R01-R09）都只跑了一轮（agent-browser 同日两轮）。为收方差，我又补跑了一轮：2026-06-20 我用 **Claude Code 主控、每工具一个干净 Subagent、顺序复用同一台 9223 测试 Chrome**（playwright-cli 在这轮 attach 9223 崩、改用自管浏览器；该崩当时记为 service_worker 断言，2026-06-22 复测已不复现，见 4.7），把外场 R01-R09 和靶场 T01-T20 **各独立跑了两轮**专门收方差。这轮只比四个真实 CLI/MCP 工具（Codex 专属 `@chrome`/`@browser` 无等价物，未纳入），与同日 Codex 轮互不参考。
...
<tr class="dt-flow"><td>playwright-cli</td><td>9 N-R</td><td>9 N-R</td><td>两轮稳定复现 <code>connectOverCDP</code> attach 失败、连接建不起来（当时记为撞扩展 <code>service_worker</code> 断言；2026-06-22 同版本复测已不复现，见 4.7）</td></tr>
</tbody></table></div>
</figure>
...
- **读不到的**：`@browser`（in-app 浏览器不继承真实登录态）；`playwright-cli` 在这轮 T10a/R01-R09 约束下 attach 9223 失败、当时记为枚举企业扩展的 `service_worker` target 触发 playwright-core 内部断言、daemon 直接崩。但这不是“永远不能 attach 9223”的结论：T10c 单题 `attach --cdp=http://127.0.0.1:9223` 当场成功；**2026-06-22 我又用当前 playwright-cli 0.1.14 对同一台 9223（Chrome 149）做了顺序复测，旧的 service_worker 断言已复现不出——真正决定成败的是“9223 当下有没有打开的页面”：有 page 时 attach 连跑都成功（且并存 6 个扩展 `service_worker` target 也不崩），只在 9223 没有任何 page target 时稳定崩在 `Browser.setDownloadBehavior: Browser context management is not supported`。** 也就是说，playwright-cli attach 真实 9223 的成败随版本/环境/浏览器状态漂移，不该当成固定的能力结论；这也是为什么前文只把它定位成“自启浏览器回归测试首选、不是真实 profile 主力”。
...
<section class="tw-panel pan5" style="--tw-c:#54579a" aria-label="playwright-cli 实现链路"><div class="tw-pane-h"><span class="tw-pane-name">playwright-cli</span><span class="tw-pane-arch">工程化总入口 · Playwright 引擎之上</span></div><div class="tw-steps"><div class="tw-step"><b>你的指令</b><small>人 · CI · MCP · agent 都能调同一个入口</small></div><div class="tw-link"></div><div class="tw-step"><b>三个包</b><small>core ＋ playwright ＋ @playwright/test；命令行参数统一降维成一份配置覆盖</small></div><div class="tw-link"></div><div class="tw-step is-star"><span class="tw-flag good">可执行性检查</span><b>Playwright 引擎</b><small>actionability：动作前自动确认元素存在 · 可见 · 不再移动 · 没被遮挡；locator 每次按最新 DOM 重新找回</small></div><div class="tw-link"></div><div class="tw-step"><b>CDP / BiDi</b><small>底层协议层</small></div><div class="tw-link"></div><div class="tw-step is-warn"><span class="tw-flag warn">外场卡点</span><b>Chrome（自启浏览器）</b><small>attach 真实 9223 稳定性随版本/环境漂移（无页面时崩于 setDownloadBehavior）</small></div></div><div class="tw-why"><b>为什么是这样：</b>把"一个真人此刻能不能完成这个动作"编码进动作模型，从根上消掉手写死等待的偶发失败——这就是它一次点中视口外只露 3px 按钮的底气，综合成绩全场最佳。短板在外场：attach 真实 9223 的稳定性随版本/环境漂移——评测当轮崩在 <span class="tw-mono">service_worker</span> 断言，2026-06-22 复测已不复现（有页面即成功、无页面则崩于 <span class="tw-mono">setDownloadBehavior</span>，见 4.7），可预测性不足，所以更适合自启浏览器的回归测试。</div></section>
</div>
